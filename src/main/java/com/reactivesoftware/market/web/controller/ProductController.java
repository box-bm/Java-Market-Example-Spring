...
public class ProductController {
    ...
    @GetMapping("/{id}")
    @ApiOperation("Search a specific product with id")
    @ApiResponses({
            @ApiResponse(code = 200, message = "OK"),
    ...
    @ApiOperation("Search products with the same category")
    @ApiResponses({
            @ApiResponse(code = 200, message = "OK"),
            @ApiResponse(code = 404, message = "Not Found"),
    })
    public ResponseEntity<List<Product>> getByCategory(@ApiParam(value = "The category id", required = true, example = "1") @PathVariable("categoryId") int categoryId) {
